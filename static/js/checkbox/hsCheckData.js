(function ($) {
    $.fn.hsCheckData = function (options) {
                ...
                for (var key in opts.data[i]) {
                    if (opts.data[i][key].toString().indexOf("object") == -1) {
                        if (childIsNUll == false) {
                            ...
                            html += "<span class='text'>" + opts.data[i][key] + "</span>";
                            html += "</div></li>"
                        }
                    } else {
        ...
        function buttonHtml() {
            var html = "<div class='hcd_btn_div'><a id='customField'></a>";
            html += "<button type='button' id='" + id + "_btn' onclick='submitCustomField();' class='hcd_btn'>确定</button>";
            html += "</div>";
            return html;
        }
        //过滤父项目
